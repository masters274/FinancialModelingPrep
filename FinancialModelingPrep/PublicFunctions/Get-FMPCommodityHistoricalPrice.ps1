function Get-FMPCommodityHistoricalPrice { 
 

    <#
        .SYNOPSIS
            Retrieves historical price data for a specified commodity using the Financial Modeling Prep API.

        .DESCRIPTION
            The Get-FMPCommodityHistoricalPrice function fetches end-of-day (EOD) historical price data for a given commodity.
        ...
        .EXAMPLE
            Get-FMPCommodityHistoricalPrice -Symbol GCUSD -FromDate (Get-Date).AddMonths(-1) -ToDate (Get-Date)

            Retrieves the full historical price data for Gold (GCUSD) over the past month.

        .EXAMPLE
            Get-FMPCommodityHistoricalPrice -Symbol OUSX -Mode Light -FromDate (Get-Date "2025-01-01") -ToDate (Get-Date "2025-03-31")

            Retrieves the light historical price data for Crude Oil (OUSX) for Q1 2025.

        .NOTES
            This function uses the Financial Modeling Prep API's Commodities Historical Price EOD endpoints.
    ...
    Begin {
        ...
        if ($Mode -eq "Light") {
            $baseUrl = "https://financialmodelingprep.com/api/v3/historical-price-full/commodity/light"
        }
        else {
            $baseUrl = "https://financialmodelingprep.com/api/v3/historical-price-full/commodity"
        }
    }
    ...
    Process {
        ...
        try {
            ...
            throw "Error retrieving commodity historical price data: $_"
        }
    }
